...
Settings are stored within the `config.cfg` file and can be used to modify how the engine behaves. A custom config can be used by parsing it as an argument to the main script on launch, eg: `init.py my_config.cfg`. Below is a description of each category and setting:
  ...
  - `RENDER`: Renderer related settings used by the camera.
    ...
    - `samples_min`: Minimum number of samples. Can be lower than 1 and even negative to support probabilistic pixel skipping, eg: `samples_min = 0` with `samples_max = 1` is a 50/50 chance of redrawing each pixel per frame. Values higher than 1 enable multisampling which looks softer and reduces roughness by doing multiple traces per pixel: This has an extreme performance impact and greatly reduces FPS, consider using `samples_max` for probabilistic extra samples instead.
    - `samples_max`: Maximum number of samples. If greater than `samples_min` there's a chance of preforming anywhere up to this number of samples: The random float between the two settings is rounded to the nearest integer so new samples are introduced in steps of 0.5, for example a value larger than 1.5 starts allowing up to 2 samples. Note that sample patterns are always random even if `static` is enabled, using this will introduce slight flickering for camera effects such as DOF.
